...
class MediaController {
    static async getProductMedia(req, res) {
        ...
        if (productMedia) {
            res.status(200).json({ code: 200, message: "Data selected Successfully", data: productMedia, });
    ...
    static async create(req, res) {
        ...
        // console.log(data);
        if (data) {
            res.status(200).json({ code: 200, message: "Data Inserted Successfully", data: data, });
        } else if (temp === false) {
            res.status(500).json({
                status: false,
                message: "Unknown Error Happened"
    ...
    static async delete(req, res) {
        ...
        if (temp) {
            res.status(200).json({ status: true, message: "Element Has Been Deleted Successfully" });
        } else if (temp === false) {
            res.status(404).json({ status: false, message: "Element Not Exist" });
        } else {
            res.status(500).json({ status: false, message: "Unknown Error Happened" });
        }
    }
